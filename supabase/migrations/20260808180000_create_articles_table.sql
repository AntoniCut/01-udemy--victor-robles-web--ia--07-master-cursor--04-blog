...
create index articles_published_created_at_idx
...
create policy "Usuarios autenticados ven todos los artículos"
    on public.articles
    for select
    to authenticated
    using (true);

create policy "Usuarios autenticados crean artículos"
    on public.articles
    for insert
    to authenticated
...
create policy "Autores eliminan sus artículos"
    on public.articles
    for delete
    to authenticated
    using (auth.uid() = author_id);
